...
$stmt = $pdo->prepare("
    SELECT DISTINCT u.id, u.full_name, u.role, u.department 
    FROM users u 
...
if ($selectedUserId) {
    ...
    if (!$activeStudent) {
        $studentStmt = $pdo->prepare("SELECT id, full_name, role, department FROM users WHERE id = ? AND role = 'student'");
            ...
            <div style="padding:10px 15px; border-bottom:1px solid #d1d7db; background:#fff;">
                <form method="GET" action="support.php">
                    <select name="user_id" onchange="this.form.submit()" style="width:100%; padding:8px; border-radius:6px; border:1px solid #d1d7db; font-size:13px; outline:none; background:#f9f9f9; cursor:pointer;">
                        ...
                        <?php 
                        $allStudentsStmt = $pdo->query("SELECT id, full_name, department FROM users WHERE role = 'student' ORDER BY full_name");
            ...
            <?php foreach($students as $s): ?>
                <a href="?user_id=<?php echo $s['id']; ?>" class="<?php echo $s['id'] == $selectedUserId ? 'active' : ''; ?>">
                    ...
                    <div class="contact-info">
                        ...
                        <div class="contact-role"><?php echo e($s['department'] ?? ''); ?></div>
                    </div>
                </a>
            <?php endforeach; ?>
                ...
                if (data.messages) {
                    if (data.messages.length === 0) {
                        ...
                        data.messages.forEach(function(m) {
                            ...
                            if (!isAdminMsg) { html += '<div class="msg-sender">' + escapeHtml(m.sender_name) + '</div>'; }
